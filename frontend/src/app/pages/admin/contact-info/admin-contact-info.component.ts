    ...
    <div class="container">
      <mat-card>
        <mat-card-header>
          ...
          <mat-card-subtitle>Téléphone, email, adresse</mat-card-subtitle>
        </mat-card-header>
        <mat-card-content>
...
export class AdminContactInfoComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private api: ApiService, private snack: MatSnackBar) {
    this.form = this.fb.group({ address: ['', Validators.required], phone: ['', Validators.required], email: ['', [Validators.required, Validators.email]] });
  }
  ngOnInit(): void {
  ...
  save() {
    ...
    this.api.updateBusinessInfo(v).subscribe({
      next: () => { this.isSaving = false; this.snack.open('Coordonnées enregistrées', 'Fermer', { duration: 3000 }); },
