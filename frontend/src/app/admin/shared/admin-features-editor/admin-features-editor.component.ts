...
@Component({
  selector: 'app-admin-features-editor',
  standalone: true,
  ...
  template: `
    <div class="editor">
      <div class="header">
        ...
        <button class="btn" type="button" (click)="add()">Ajouter une fonctionnalité</button>
      </div>
      <div class="list">
        <div class="item" *ngFor="let g of features.controls; let i = index" [formGroup]="$any(g)">
          <div class="item-header">
            <div class="item-index">{{ i + 1 }}</div>
            <div class="item-actions">
              <button class="btn-secondary" type="button" (click)="moveUp(i)" [disabled]="i===0">↑</button>
              <button class="btn-secondary" type="button" (click)="moveDown(i)" [disabled]="i===features.length-1">↓</button>
      ...
      <div class="actions">
        <button class="btn" type="button" (click)="save()">Enregistrer les fonctionnalités</button>
      </div>
    </div>
  `,
  styles: [`
...
export class AdminFeaturesEditorComponent implements OnInit {
  form: FormGroup;
  icons = ['award','users','clock','check-circle','shield','heart','book-open','trending-up'];
  constructor(private fb: FormBuilder, private api: ApiService, private snack: MatSnackBar) {
    this.form = this.fb.group({ features: this.fb.array([]) });
  }
  get features(): FormArray {
    return this.form.get('features') as FormArray;
  }
  add(item: any = { icon: 'award', title: '', description: '' }) {
    this.features.push(this.fb.group({ icon: [item.icon || 'award'], title: [item.title || ''], description: [item.description || ''] }));
  }
  remove(i: number) { this.features.removeAt(i); }
  moveUp(i: number) {
    if (i === 0) return;
    const tmp = this.features.at(i);
    this.features.removeAt(i);
    this.features.insert(i - 1, tmp);
  }
  moveDown(i: number) {
  ...
  ngOnInit(): void {
    this.api.getContent('services','features').subscribe({
      next: (i) => {
        try {
          const arr = JSON.parse(i?.content || '[]');
          (arr || []).forEach((it: any) => this.add(it));
          if (this.features.length === 0) this.add();
        } catch { this.add(); }
      },
      error: () => { this.add(); }
    });
  ...
  save() {
    const payload = JSON.stringify(this.features.value);
    this.api.saveContent({ page_name: 'services', section_name: 'features', content_type: 'json', content: payload }).subscribe({
      next: () => this.snack.open('Fonctionnalités enregistrées', 'Fermer', { duration: 3000 }),
